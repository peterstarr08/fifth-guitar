function generateNote(id, chord, highlighted, accented, clickHandler){
    if(highlighted){
        if(accented){
            return <div  id={"ab"} className="  border-l-4 border-l-sky-600 bg-slate-200 w-20 h-8 pl-1 hover:bg-slate-300" onClick={clickHandler}>{chord}</div>
        }
    ...
    if(accented){
        return <div  id={id} className=" border-l-4 border-l-sky-600  w-20 h-8 pl-1 hover:bg-slate-300" onClick={clickHandler}>{chord}</div>
    }
